...
public class RegisterHandler extends AbstractHandler {
    ...
    public RegisterHandler(final UserManager userManager){
    ...
    protected String processRequest(final HttpExchange httpExchange, final String method) {
        ...
        if(POST.equals(method)){
            ...
            try{
                ...
                final String registerErrorMessage = _userManager.createNewUser(username,pass);
                if(registerErrorMessage == null){
                    response = Utils.createJsonReply(true,Instruccion.INSTRUCCION_DESCONOCIDA,"Registrado OK!");
                    responseAsString = response.toString();
                } else {
                    response = Utils.createJsonReply(false,Instruccion.INSTRUCCION_DESCONOCIDA,registerErrorMessage);
                    responseAsString = response.toString();
                }
